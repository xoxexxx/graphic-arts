...
export const EditorMenu: React.FC = () => {
    ...
    return(
        <div className="editor-menu flex flex-col items-center w-24 h-full bg-[#9a77e5]">
            {tabMenu.map((tab) => (
                <div key={tab.id} data-action={tab.action} className={twMerge(currentMenu === tab.action && 'active', "mx-4 my-3 p-1.5 rounded-2xl diration-500 cursor-pointer hover:bg-[#000000]/20 first:mx-4 first:mt-6 first:mb-2")} 
                onClick={handler}>
