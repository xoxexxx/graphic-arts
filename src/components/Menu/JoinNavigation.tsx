    ...
    return(
        <div className={twMerge( `min-w-[280px] max-[1140px]:hidden flex flex-col`, !isLogin && `hidden` || drop_ && 'd-trsfm' || drop && `!block`) }>
            <ul className="m-0 pl-5">
                {drop_ && <img src={logo} alt="creatiqa_menu" className="cursor-pointer mb-10" onClick={hidden}  /> }
                ...
                <li className={twMerge(drop && 'hidden', 'w-52', "whitespace-nowrap")} onClick={hidden}> <NavLink  className="bg-[#F3F3F3] rounded-xl w-56 duration-300 text-black block mt-2.5 py-2 px-4" to="/premium"> <img className="mr-2.5  -translate-y-0.5  inline" src={i4} alt="creatiqa_menu" /> Creatiqa Premium</NavLink></li>
            </ul>
