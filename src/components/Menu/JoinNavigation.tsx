    ...
    return(
        <div className={twMerge( `min-w-[280px] max-[1140px]:hidden flex flex-col`, !isLogin && `hidden` || drop_ && 'd-trsfm' || drop && `!block`) }>
            <ul className="m-0 pl-5">
                ...
                <li className="w-52" onClick={hidden}> <NavLink className="rounded-xl w-56 text-black block mt-2.5 py-2 px-4" to="/home"><img className="-translate-x-2 inline" src={i1} alt="creatiqa_menu" /> Главная</NavLink> </li>
                <li className="w-52" onClick={hidden}> <NavLink className="rounded-xl w-56 text-black block mt-2.5 py-2 px-4" to="/templates"><img className="-translate-x-2 inline" src={i2} alt="creatiqa_menu" /> Шаблоны</NavLink> </li>
                <li className="w-52" onClick={hidden}> <NavLink className="rounded-xl w-56 text-black block mt-2.5 py-2 px-4" to="/projects"> <img className="-translate-x-2 inline" src={i3} alt="creatiqa_menu" /> Проекты</NavLink></li>
                <li className={twMerge(drop && 'hidden', 'w-52')} onClick={hidden}> <NavLink  className="bg-[#F3F3F3] rounded-xl w-56 text-black block mt-2.5 py-2 px-4" to="/premium"> <img className="mr-2.5 -translate-x-2  inline" src={i4} alt="creatiqa_menu" /> Creatiqa Premium</NavLink></li>
            </ul>
            <div className={twMerge('flex flex-col', isProfile && `hidden`, !drop && `hidden`,)}>
